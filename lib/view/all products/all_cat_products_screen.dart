// ignore_for_file: use_key_in_widget_constructors, must_be_immutable, prefer_typing_uninitialized_variables, non_constant_identifier_names, no_logic_in_create_state
...
class AllCatProductsScreen extends StatefulWidget {

  var cat_id;
  AllCatProductsScreen(this.cat_id);

  @override
  State<AllCatProductsScreen> createState() => _AllCatProductsScreenState(cat_id);
}

class _AllCatProductsScreenState extends State<AllCatProductsScreen> {

  var cat_id;
  _AllCatProductsScreenState(this.cat_id);
  ...
  void initState() {
    super.initState();
    AppCubit.get(context).GetProductsByCat(token: access_token, page: "1", cat_id: cat_id);
  ...
  void _scrollListener(){
    if(scrollcontroller.position.pixels == scrollcontroller.position.maxScrollExtent){
      if(page < totalPages){
        ...
        page = page + 1;
        AppCubit.get(context).GetProductsByCat(token: access_token, page: page.toString(), cat_id: cat_id);
      }
    }
  }
}

...
class AllHomeProductsScreen extends StatefulWidget {

  @override
  State<AllHomeProductsScreen> createState() => _AllHomeProductsScreenState();
}

class _AllHomeProductsScreenState extends State<AllHomeProductsScreen> {
  ...
  void initState() {
    super.initState();
    AppCubit.get(context).GetAllProducts(token: access_token, page: "1");
    ...
    return BlocConsumer<AppCubit, AppStates>(
      listener: (context, state){
        if(state is GetAllProductsSuccessState){
          ...
          isLoadingMore = false;
          totalPages = state.allProductsModel.totalPages!;
  ...
  void _scrollListener(){
    if(scrollcontroller.position.pixels == scrollcontroller.position.maxScrollExtent){
      if(page < totalPages){
        ...
        page = page + 1;
        AppCubit.get(context).GetAllProducts(token: access_token, page: page.toString());
      }
    }
  }
}
